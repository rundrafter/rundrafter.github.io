...
| `current_fitness` | optional  | weekly_distance, longest_run, recent_peak_weekly (opt) — a runner new to running with no honest numbers to give may leave the whole group blank (ADR 018); `periodise` falls back to a beginner starting-volume default                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `weekly_schedule` | optional  | availability grid (morning/evening per weekday, all ticked by default), preferred_sessions — the **weekly session template** (repeating): day, type (broad-type enum), description (opt), distance_min/distance_max (opt; `distance_max: 0` = no running), tailored (opt, default true — the "skip tailoring" tickbox emits `false`). No `rest_days` override any more (ADR 017) — the resolver always derives rest days. No separate `long_run_day` override either (ADR 019) — a `type: "long"` template entry is the only way to pin the long-run day, and at most one is allowed. Absent entirely leaves the schedule to the resolver. |
...
- **Empty-object pruning.** Never emit an optional section as an empty or
  partial object. If the runner leaves `runner`, `recent_result`,
  `current_fitness`, `b_races`, `other_events`, or `notes` blank, omit the key
...
- **Schedule** (mirrors `_validate_schedule`, checked only against the
  *explicit overrides* the runner gave — `days_available` and `rest_days`
  ...
  - A day whose grid has both halves unticked (see `pruneAvailability`'s
    ...
    - At most one `type: "long"` entry is allowed in
      `preferred_sessions[]` (`MULTIPLE_LONG_RUN_ENTRIES`); there is no
      separate `long_run_day` field any more (ADR 019) — a `long`-type
      entry is the only way to pin the long-run day.
    - If a `type: "long"` entry is present, its `day` must not be a
      fully-unticked day (`LONG_RUN_DAY_UNAVAILABLE`).
